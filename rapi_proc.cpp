...
int EvseRapiProcessor::processCmd()
{
  ...
  case 'G': // get parameter
	...
	u2.i = MAX_CURRENT_CAPACITY_L1;
      ...
      rc = 0;
      break;
 #ifdef DELAYTIMER     // added delay timer status - Bobby Chung
    case 'D': // get delay timer start and stop times and IsTimerEnabled flag
      extern DelayTimer g_DelayTimer;
      sprintf(buffer,"%d %d %d %d %d",(int)g_DelayTimer.GetStartTimerHour(),(int)g_DelayTimer.GetStartTimerMin(),(int)g_DelayTimer.GetStopTimerHour(),(int)g_DelayTimer.GetStopTimerMin(),(int)g_DelayTimer.IsTimerEnabled());
      bufCnt = 1; // flag response text output
      rc = 0;
      break;
#endif // DELAYTIMER  
